...
//  let fdata3 = {title : data3.title};
  ...
  describe('> WovClientLocal', async function() {
    ...
    it('> client _runSingularQuery', async function() {
      ...
      let d1 = ['testname'];
      let result1 = await cl._runSingularQuery(q1, d1, 'WovClientLocalTest');
      expect(result1.name).to.equal(d1[0]);

      let q2 = `DELETE FROM testtable WHERE id = $1::integer RETURNING id`;
      let d2 = [result1.id];
      let result2 = await cl._runSingularQuery(q2, d2, 'WovClientLocalTest');
    ...
    it('> client _runUpdateQuery', async function() {
      ...
      let result2 = await cl._runUpdateQuery(q2, d2, 'WovClientLocalTest');
      ...
      let result3 = await cl._runUpdateQuery(q3, d3, 'WovClientLocalTest');
